...
describe('o-date', () => {
	...
	describe('ftDateFormat.timeAgo', () => {
		let clock;
		beforeEach(() => {
			clock = sinon.useFakeTimers(new Date('Jul 13 2016 10:02:49'));
		});
	...
	describe('ftDateFormat.asTodayOrYesterdayOrNothing', () => {
		...
		it('returns "today" if isToday returns true', () => {
			try {
				...
				proclaim.isInstanceOf(call.args[1], Date);
		...
		it('returns "yesterday" if isYesterday returns true AND isToday is false', () => {
			try {
				...
				proclaim.isInstanceOf(todayCall.args[1], Date);
				...
				proclaim.isInstanceOf(yesterdayCall.args[1], Date);
